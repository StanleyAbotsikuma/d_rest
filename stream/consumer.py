...
class TestConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        # Join project
        await self.channel_layer.group_add(self.project_name,
                                           self.channel_name)
    ...
    async def disconnect(self, close_code):
        # Leave project
        await self.channel_layer.group_discard(self.project_name,
                                               self.channel_name)
...
# Receive message from WebSocket
    ...
    async def receive(self, text_data):
        ...
        # Send message to current project
        await self.channel_layer.group_send(self.project_name, {
            'type': 'iot_message',
            'message': message
        })
    ...
    async def iot_message(self, event):
        ...
        try:
            if (message["receive_type"] == "device"):
                ...
                await self.send(
                    text_data=json.dumps({'device': message["device_name"]}))
        except:
            pass
        ...
        try:
            if (message["receive_type"] == "de_device"):
                ...
                await self.send(
                    text_data=json.dumps({'de_device': message["device_name"]})
                )
        except:
            pass
